...
def aide(): # fourni les liens URLs de la plateforme d'aide en ligne, du pdf et de la page youtube
    ...
    root.geometry("%dx%d" % (600,170))
    label0=Label(root,justify='left',bg=gris_5,font=("Arial", 14, "bold italic"),text='!? Besoin d\'Aide ?!\n')
    ...
    label8=Label(root,justify='center',bg=gris_5,font=font_texte2,text= '3/ (Bientôt) Rendez-vous sur le site de développement du logiciel et posez vos questions à\n la communauté sur le forum dédié')
...
def credit(): # annonce la version du programme, les concepteurs du programme et l'année de développement
    ...
    root.config(bg=gris_5)
    root.wm_iconbitmap('images/petitlogo1.ico')
    root.geometry("%dx%d" % (700,520))
    label0=Label(root,justify='center',bg=gris_5,font=("Arial", 14, "bold italic"),text='SOLVE STRUCTURE --- VERSION 1.0')
    label0.pack(fill='both')
    label8=Label(root,justify='center',bg=gris_5,font=font_texte2,text= 'A la demande de l\'EPF, une equipe d\'étudiants a mis en place le logiciel Solve Structure qui est\nun programme ayant pour intérêt d\'étudier les effets d\'une ou plusieurs charges sur une structure.')
    label8.pack(fill='both')
    label2=Label(root,justify='center',bg=gris_5,font=font_texte2,text= 'L\'application a été conçu par :\n')
    label2.pack(fill='both')
    ...
    label6.pack(fill='both')
    label8=Label(root,justify='center',bg=gris_5,font=("Arial", 14, "bold italic"),text='Remerciements')
    label8.pack(fill='both')
    label10=Label(root,justify='center',bg=gris_5,text='Nous remercions le professeur Omar SAIFOUNI de nous avoir proposé et accompagné\ntout au long de ce projet de création de logiciel de structure.')
    label10.pack(fill='both')
    label12=Label(root,justify='center',bg=gris_5,text='Nous remercions le professeur Maxime PIGEOT de nous avoir apporter un soutien\ntechnique pour concevoir le logiciel.')
    label12.pack(fill='both')
    CHEMIN = os.getcwd()
